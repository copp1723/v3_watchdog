"""
Consolidated formatter for structuring AI insight outputs in Watchdog AI.

Provides schema validation, markdown formatting, and output parsing.
Combines features from both the original and improved versions.
"""
...
class InsightOutputFormatter:
    ...
    def format_response(self, response_text: str) -> Dict[str, Any]:
        ...
        """
        try:
            # If response_text is already a dict, convert to string first
            ...
            try:
                ...
                logger.debug("Not valid JSON, trying to extract from text")
    ...
    def _validate_and_complete(self, response: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for field in self.required_fields:
            if field not in response or not response[field]:
                logger.warning(f"Required field '{field}' missing or empty in response")
    ...
    def _apply_markdown_formatting(self, insight: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for item in formatted['value_insights']:
            # Skip processing if already well-formatted
            if isinstance(item, dict):
                formatted_insights.append(item)
                continue
                
            # Add bold for metrics and key terms
            item_metadata = extract_metadata(str(item))
            formatted_item = format_markdown_with_highlights(str(item), item_metadata.highlight_phrases)
    ...
    with st.container():
        ...
        with st.container():
            ...
            if insight.get("is_error"):
                st.error(insight.get("error", "An unknown error occurred"))
